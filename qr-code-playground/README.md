...
- 使用 Python 语言。当然，如果你没有 Python 基础，可能会比较困难，那么你可以使用其他你熟悉的语言。
- 规范化编程。规范化的编程不仅可以增加代码的可读性，还可以避免一些意料之外的 bug。
- 模块化编程。模块化的编程便于调试，也可以使代码更易读。
- 使自己的代码文件可以供他人作为库导入。为此，你撰写的文档应当足够明晰。
...
- Alphanumeric mode: 适用于编码仅含有阿拉伯数字、大小写字母与一些常用符号的字符串。**本次我们只考虑这种编码方式。**
...
可选的掩码图案有 7 种。需要依次评估每个掩码的罚分（Penalty）。具体来说，有 4 个评估测试。分别是：
...
每条检测都规定的检测方式与罚分。最后应在 7 种掩码图案中选取罚分最少的那一种。
...
这份题目描述其实隐去了很多具体细节。希望大家可以自己利用网络寻找自己需要的信息。

如果对本题有什么疑问或者何处表述有错误，请联系@保安 。

需要重申的是，我们最终考察的并非只是题目完成度，更多的是考察学习能力与态度。如果实现全部功能比较困难，可以只实现其中一部分。

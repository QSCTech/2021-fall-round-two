...
- 你可以把要补充代码写在任何地方(当然我们希望你还是带着前缀的), 比如一个`.ts`文件, 或许一个`.md`文件, 甚至一张纸的照片也可以, 然后按规定进行提交
- 按自己能力尽力而为就好, 主要是要在这段经历中锻炼自己的思维能力, 完成度绝对不是我们考察的最重要的标准
- **文末的问题必答**
- Have fun~ :-D
...
当然这个太简单了没什么意思, 我们来尝试使用 TypeScript~~强大的~~类型系统, 来实现一个判断素数的类型 IsPrime

### 效果

```ts
type check = IsPrime<18>; // type check = "F"
...
在 TypeScript 中, 字面量也可以作为类型, 此时, 这个值就只能是这个字面量
...
- 首先, 它可以表示包含关系, 从集合的观点来看, `A extends B`表示 A 是 B 的子集(当然**不是**真子集). 在这种情况下, 类型为 A 的变量可以直接赋值给类型为 B 的变量
- 其次, 在泛型中, 我们可以定义
  ```ts
...
然后我们根据这个, 可以很轻松的定义出找一个数的后继的类型和找一个数的前驱的类型, 这里请大家完成
...
接下来请大家理解上面干了啥, 然后参照着实现减法 乘法 和整除
